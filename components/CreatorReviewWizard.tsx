...
export function CreatorReviewWizard() {
  const [request, setRequest] = useState<CreatorReviewRequest>(emptyRequest);
  ...
  const questions = useMemo(
    () => [
      ...
      },
      {
        ...
        helper:
          "Add the creator handle, channel name, or personal brand being reviewed.",
      ...
      },
      {
        ...
        helper:
          "Share the pages we can review — your YouTube, Instagram, TikTok, website, etc.",
      ...
      },
      {
        ...
        helper: "Choose the closest match.",
        isComplete: request.creatorDescription.trim().length > 0,
        field: (
          <select
            value={request.creatorDescription}
            onChange={(e) =>
              setRequest((current) => ({
                ...current,
                creatorDescription: e.target.value,
              }))
            }
            className={inputClass}
            required
          >
            <option value="">Select one</option>
            {creatorDescriptionOptions.map((option) => (
              <option key={option} value={option}>
                {option}
              </option>
            ))}
  ...
  const submit = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/form-submissions", {
        ...
        body: JSON.stringify({
          formType: "Creator Review request",
          fields: {
            name: request.name,
            brand: request.brand,
            roleType: request.roleType,
      ...
      console.log("[CreatorReviewWizard] creator review request saved", {
        email: request.email,
      });
      setSubmitted(true);
    } catch (error) {
      console.error("[CreatorReviewWizard] creator review request failed", {
        error,
      });
      setSubmitError(
  ...
  const goNext = useCallback(async () => {
    console.log("[CreatorReviewWizard] advancing step", {
      from: step,
      to: step + 1,
    ...
    >
      ...
      <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <button
          type="button"
          onClick={() => {
            console.log("[CreatorReviewWizard] returning step", {
              from: step,
              to: step - 1,
            });
